...
var fileUtilities = {
	downloadOpenPDf: function (url,folder,fileName) {
	    window.resolveLocalFileSystemURL(
				...
				function(){//función errorCallback: si el fichero NO está en la ubicación
		    		var ft=new FileTransfer();//Crear objeto FileTransfer
		    		ft.download(url,folder+fileName,//Descargar fichero indicado por URL al folder+fileName indicado
		    			function() {//función successCallback: si el fichero se descargó bien
							alert("Fichero descargado en "+folder+fileName);//indicar al usuario que el fichero ha sido descargado en folder+fileName
							cordova.InAppBrowser.open("file://"+folder+fileName,"_system","location=yes");//abrir nueva ventana del navegador del sistema, para mostrar el fichero
		    		},
						function(error){//función errorCallback: si el fichero NO se descargó bien
							alert("Error al descargar: "+error.code);//indicar al usuario el error de descarga
						}
		    		);
		    	}
	    );		
	...
	downloadOpenZIP: function (url,folder,fileName) {
		window.resolveLocalFileSystemURL(
				"file://"+folder+fileName,//Comprobar si el fichero está en folder+fileName
				function(){//función successCallback: si el fichero está en la ubicación
					zip.unzip("file://"+folder+fileName,folder,alert("Fichero descomprimido"));//descomprimir en la misma carpeta e indicarlo cuando se termine
				},
				function(){//función errorCallback: si el fichero NO está en la ubicación
					var ft=new FileTransfer();//Crear objeto FileTransfer
		    		ft.download(url,folder+fileName,//Descargar fichero indicado por URL al folder+fileName indicado
