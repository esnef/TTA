/*
 * $Id: objects.js Oct 16, 2015 10:07:43 PM tta1516$
 * 
...
var appConstants = {
	localPermanentStorageFolder: "/sdcard/eus.ehu.INTEL901_504021.TTA1516_LS_EX_09v4/",
	...
	uploadFileURL: "http://158.227.64.60:8080/example9S/rest/School/uploadFile" //EHU PUBLIC
};
...
//		alert("create1");
		...
		var headerDiv=
			'<div data-role="header" style="padding-bottom:1%" data-position="fixed" data-fullscreen="false">'+
				'<h1 style="margin-left:0;margin-right:0;white-space: nowrap;overflow: visible;">TTA1516_LS-EX_09v4: exercises4</h1>'+
			'</div>';
...
var photo = {
		...
		}//Método asíncrono para tomar foto
}
...
var video = {
		fileFolder:null,
		fileName:null,
		recordAsync: function(fileFolder,fileName,onSuccess) {
			navigation.device.capture.captureVideo(//Grabar un video
				function(videoFiles) {//función successCallback: si se grabó el video
					var tempFullPath=videoFiles[0].fullPath;//Ubicación del fichero del video grabado
					
					tempFullPath=tempFullPath.substring(tempFullPath.indexOf("/"));
					alert("New video in: "+tempFullPath);
					
					fileUtilities.moveAsync(tempFullPath,fileFolder,fileName,//Mover el fichero del video grabado a fileFolder+fileName
				        function() {//función successCallback: si se movió el fichero
							video.fileFolder=fileFolder;//Guardar en el atributo fileFolder del objeto video, la carpeta destino
							video.fileName=fileName;//Guardar en el atributo fileName del objeto video, el nuevo nombre del fichero
							if(onSuccess!=false)
								onSuccess();
		        		}							
					);
				...
				function(error) {
					var msgText = "Video error: " + error.message + "(" + error.code + ")";
					alert(msgText);
				}
			);
		}
